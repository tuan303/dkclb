...
function findCardWarning(club) {
  ...
  if (daGui.some((don) => don.id === club.id)) {
    return { nhan: "Con đã đăng ký chính lớp này", nut: "Đã đăng ký", khoa: true };
  }
  // Nhiều lớp của cùng một CLB thì được (yêu cầu giáo vụ 11/09/2026). Trùng giờ thì
  // KHÔNG khoá nút nữa: bấm vào sẽ hỏi phụ huynh có muốn đổi sang lớp này không.
  const vuongGio = findScheduleConflict(club);
  if (!vuongGio) return null;
  return doiDuocKhiTrung(vuongGio)
    ? { nhan: conflictBadge(vuongGio.doiThu), nut: "Đổi sang lớp này", khoa: false }
    : { nhan: `${conflictBadge(vuongGio.doiThu)} — lớp đó đã đóng phí, liên hệ nhà trường nếu muốn chuyển`, nut: "Trùng giờ", khoa: true };
}

/** Đơn đã gửi đang vướng giờ (nếu có) và nó còn đổi được không. Trong giỏ thì luôn đổi được. */
function donVuongGio({ doiThu, daDangKy }) {
  return daDangKy
    ? state.registrations.find((don) => don.classId === doiThu.id && don.studentId === state.studentId
      && ACTIVE_REGISTRATION_STATUSES.includes(don.status)) || null
    : null;
}
function doiDuocKhiTrung(vuongGio) {
  if (!vuongGio.daDangKy) return true;
  const don = donVuongGio(vuongGio);
  return Boolean(don && DOI_LOP_DUOC.includes(don.status) && !don.feePaid);
}

/** Đơn phụ huynh còn tự đổi lớp được — khớp DOI_LOP_DUOC ở registration-status.mjs. */
const DOI_LOP_DUOC = ["submitted", "waitlist", "payment"];

/** Lớp này phụ huynh còn bấm chọn được không, và nếu không thì vì sao. */
function lyDoKhongChonDuoc(club) {
  const ma = club.trangThaiLop?.ma;
  if (ma === "dung-tuyen") return "Lớp này đã dừng tuyển.";
  if (ma === "da-du" && !club.waitlistEnabled) return "Lớp này đã đủ và không nhận danh sách chờ.";
  return null;
}

/**
 * Chọn một lớp trùng lịch → hỏi ngay tại chỗ có đổi sang lớp này không (yêu cầu giáo
 * vụ 11/09/2026). Lớp trùng đang ở trong giỏ thì đổi trong giỏ. Lớp trùng là đơn đã
 * gửi thì chỉ đổi được khi đơn đó CHƯA đóng phí; đã đóng phí thì chuyển lớp là việc
 * của nhà trường (hoàn/chuyển phí), nên chỉ báo phụ huynh liên hệ.
 */
function hoiDoiLop(target, { doiThu, daDangKy }) {
  const tenMoi = `${target.name}${target.className ? ` · ${target.className}` : ""}`;
  const donCu = donVuongGio({ doiThu, daDangKy });
  const doiDuoc = doiDuocKhiTrung({ doiThu, daDangKy });
  const vaoDsCho = target.trangThaiLop?.ma === "da-du";
  const noiDung = daDangKy ? "con đã đăng ký" : "đang có trong giỏ đăng ký";
  showModal(`<div class="modal-head"><div><span class="eyebrow">Trùng lịch học</span><h2>Đổi sang lớp này?</h2></div><button class="icon-button" data-close-modal>${icon("x")}</button></div>
    <div class="modal-body">
      <p>Lớp <b>${escapeHtml(tenMoi)}</b> (${escapeHtml(target.schedule)}) trùng lịch với <b>${escapeHtml(doiThu.name)}</b>${doiThu.schedule ? ` (${escapeHtml(doiThu.schedule)})` : ""} mà ${noiDung}.</p>
      ${doiDuoc
        ? `<p>Phụ huynh có muốn đổi sang lớp <b>${escapeHtml(tenMoi)}</b> không? Lớp cũ sẽ được bỏ${daDangKy ? " và đơn mới được gửi ngay" : " khỏi giỏ"}.</p>
          ${vaoDsCho ? `<div class="inline-alert"><span>Lớp <b>${escapeHtml(tenMoi)}</b> đã đủ: con sẽ vào <b>danh sách chờ</b> của lớp mới${daDangKy ? ", và đơn lớp cũ sẽ không còn" : ""}.</span></div>` : ""}
          ${daDangKy ? `<label class="confirm-row"><input id="doi-lop-dong-y" type="checkbox" /><span>Tôi đã kiểm tra lịch học, mức phí và đồng ý với quy định đổi/hủy của nhà trường.</span></label>` : ""}`
        : `<div class="inline-alert"><span>Lớp cũ đã đóng phí hoặc đã được xếp nên không tự đổi được. Vui lòng liên hệ nhà trường để chuyển lớp.</span></div>`}
    </div>
    <div class="modal-foot"><button class="button button-secondary" data-close-modal>Giữ lớp cũ</button>
      ${doiDuoc ? `<button class="button button-primary" data-doi-lop ${daDangKy ? "disabled" : ""}>Đổi sang lớp này</button>` : ""}</div>`);
  const nut = $("[data-doi-lop]");
  $("#doi-lop-dong-y")?.addEventListener("change", (event) => { if (nut) nut.disabled = !event.target.checked; });
  nut?.addEventListener("click", async () => {
    if (!daDangKy) {
      state.cart = state.cart.filter((id) => id !== doiThu.id);
      closeModal();
      addToCart(target.id);
      return;
    }
    nut.disabled = true;
    try {
      const ketQua = await api(`/registrations/${encodeURIComponent(donCu.id)}/doi-lop`, {
        method: "POST", body: JSON.stringify({ classId: target.id, acceptedTerms: true }),
      });
      const vaoCho = ketQua.registrations?.[0]?.status === "waitlist";
      ...
      closeModal();
      renderApp();
      toast(vaoCho ? `Đã đổi sang lớp ${tenMoi} — con đang ở danh sách chờ.` : `Đã đổi sang lớp ${tenMoi}.`, "success");
    } catch (error) {
      toast(error.details?.map((item) => item.message).join(" ") || error.message, "error");
      nut.disabled = false;
    }
  });
}
  ...
  return `
    ...
    <section class="hero">
      <div class="hero-content">
        ...
        <p>${open
          ? "Chọn học sinh, xem CLB phù hợp và hoàn tất đăng ký trong một quy trình có kiểm tra lịch học, trạng thái lớp và điều kiện."
...
function renderSteps(current) {
  const items = [["Chọn học sinh","Đúng hồ sơ"],["Chọn CLB","Theo điều kiện"],["Kiểm tra","Lịch & trạng thái lớp"],["Xác nhận","Nhận mã đơn"]];
...
// Nhãn trạng thái lớp → lớp màu có sẵn của .club-status.
const LOP_TRANG_THAI_LOP = { "con-nhan": "", "sap-du": "warning", "da-du": "full", "dung-tuyen": "full" };

function renderClubCard(club) {
  // Phụ huynh chỉ thấy TRẠNG THÁI lớp, không thấy sĩ số hay số chỗ còn (yêu cầu giáo
  // vụ 11/09/2026) — máy chủ cũng không gửi các con số đó cho phụ huynh.
  const trangThai = club.trangThaiLop || { ma: "con-nhan", nhan: "Còn nhận" };
  ...
  const khongChon = lyDoKhongChonDuoc(club);
  const khoaNut = inCart || canhBao?.khoa || Boolean(khongChon);
  const nhanNut = inCart ? "Đã chọn"
    : trangThai.ma === "dung-tuyen" ? "Dừng tuyển"
    : trangThai.ma === "da-du" && !club.waitlistEnabled ? "Đã đủ"
    : canhBao ? canhBao.nut
    : trangThai.ma === "da-du" ? "Vào DS chờ" : "Chọn";
  return `<article class="club-card">
    <div class="club-visual visual-${club.visual}"><span class="club-status ${LOP_TRANG_THAI_LOP[trangThai.ma] ?? ""}">${escapeHtml(trangThai.nhan)}</span><span class="club-symbol">${club.emoji}</span></div>
    <div class="club-body">
...
function renderRegistrations() {
  ...
  return `
    <div class="kpi-strip"><div class="kpi-item"><span>Tổng đăng ký</span><strong>${currentRegistrations.filter((don) => don.status !== "doi_lop").length}</strong></div><div class="kpi-item"><span>Đã đóng phí</span><strong>${currentRegistrations.filter(r => r.status === "confirmed").length}</strong></div><div class="kpi-item"><span>Đang học</span><strong>${currentRegistrations.filter(r => r.status === "dang_hoc").length}</strong></div><div class="kpi-item"><span>Xếp chờ</span><strong>${currentRegistrations.filter(r => r.status === "waitlist").length}</strong></div></div>
...
const STATUS_GUIDE = {
  ...
  waitlist: "Lớp đã đủ. Nhà trường sẽ liên hệ nếu có chỗ trống. Nếu con đã đóng phí, khoản phí vẫn được giữ nguyên.",
  conflict: "Lịch học bị trùng. Vui lòng gửi yêu cầu hỗ trợ để chọn ca khác.",
  cancelled: "Đơn đã hủy.",
  doi_lop: "Phụ huynh đã đổi đơn này sang lớp khác. Không cần đóng phí cho lớp này.",
  draft: "Đơn chưa gửi.",
};
...
function renderSupport() {
  const options = state.registrations.filter((item) => item.studentId === state.studentId && item.status !== "doi_lop").map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.id)} · ${escapeHtml(item.club)}</option>`).join("");
...
/** Số ca đang mở của một CLB — trong một đợt, hoặc ở MỌI đợt khi không nói đợt nào. */
function soCaDangMo(clubId, periodId = null) {
  return (state.catalog?.classes || []).filter((row) => row.clubId === clubId && row.active !== false
    && (!periodId || row.periodId === periodId)).length;
}

function clbTrungTen() {
  const theoTen = new Map();
  for (const club of state.catalog?.clubs || []) {
    // CLB đã ẩn mà còn ca đang mở vẫn tính là trùng. Ẩn CLB không đóng ca: đơn trong
    // ca đó vẫn hiệu lực, còn luật một-CLB vẫn so theo mã khác nhau. Không tính nó thì
    // ẩn nhầm một CLB còn ca là cảnh báo biến mất trong khi lỗ hổng vẫn nguyên đó.
    if (club.active === false && !soCaDangMo(club.id)) continue;
...
/**
 * Nhãn phân biệt được một CLB giữa những CLB trùng tên: mã, khối, số ca. Đếm ca trong
 * ĐÚNG đợt của ca đang sửa — cả trang này làm việc theo từng đợt, và đếm lẫn ca đợt
 * nháp là làm người gộp chọn nhầm bản ghi "chính".
 */
function nhanClbDayDu(club, periodId = null) {
  const khoi = (club.grades || []).length ? `khối ${club.grades.join(", ")}` : "chưa khai khối";
  return `${club.name} · ${club.code || club.id} · ${khoi} · ${soCaDangMo(club.id, periodId)} ca${periodId ? " trong đợt" : ""}`;
}

/**
 * Thứ tự trong ô "Thuộc CLB": theo tên ĐÃ BỎ DẤU rồi mới tới mã, đúng khoá mà
 * clbTrungTen dùng để gom. Sắp theo tên gốc thì "Bóng đá  cơ bản " và "BÓNG ĐÁ CƠ
 * BẢN" bị một CLB khác chen vào giữa, và người gộp không thấy chúng là một nhóm.
 */
function sapXepClbTheoTen(clubs) {
  return clubs.slice().sort((a, b) => khoaTenClb(a.name).localeCompare(khoaTenClb(b.name), "vi")
...
function renderClasses() {
  ...
  return `<div class="kpi-strip">
    ...
    <section class="section"><div class="section-head">
      ...
      ${orphanClubs.length ? `<div class="info-note"><strong>${orphanClubs.length} CLB chưa có lớp trong đợt này:</strong> ${orphanClubs.map((club) => {
        // Danh sách này là "chưa có ca TRONG ĐỢT ĐANG XEM". CLB còn ca ở đợt khác thì
        // không mời ẩn: ẩn là giấu luôn các ca đang nhận đơn ở đợt kia.
        const caDotKhac = soCaDangMo(club.id);
        return `<span class="orphan-club">${escapeHtml(club.name)} <span style="color:var(--muted)">(${escapeHtml(club.code || club.id)}${club.active === false ? " · đang ẩn" : ""}${caDotKhac ? ` · còn ${caDotKhac} ca ở đợt khác` : ""})</span> <span class="orphan-actions"><button class="text-button" data-add-class-for="${escapeHtml(club.id)}">+ ca</button>${caDotKhac ? "" : ` <button class="text-button" data-edit-club="${escapeHtml(club.id)}">Sửa / ẩn</button>`}</span></span>`;
      }).join(" · ")}</div>` : ""}
    </section>`;
}
 ...
 *
 * Từ khi cho một em học nhiều lớp của cùng CLB (yêu cầu giáo vụ 11/09/2026), gộp không
 * còn để chặn đăng ký trùng; nó để một CLB là MỘT bản ghi nhiều lớp — phụ huynh thấy
 * một CLB, và danh sách lớp, báo cáo gom đúng theo CLB.
 */
function renderCanhBaoClbTrungTen() {
  ...
  return `<div class="inline-alert clb-trung-ten-khoi">
    ...
    <p style="margin:6px 0 0">Mỗi CLB nên là <b>một</b> bản ghi có nhiều lớp: phụ huynh thấy một CLB thay vì nhiều CLB giống hệt tên, và danh sách lớp, báo cáo gom đúng theo CLB. Để gộp:</p>
    <ol style="margin:6px 0 0;padding-left:18px">
      <li>Chọn MỘT CLB giữ lại trong mỗi nhóm (xem mã ở dòng xám dưới tên CLB).</li>
      <li>Ở từng ca của các CLB còn lại, bấm <b>Sửa</b> → đổi ô <b>Thuộc CLB</b> sang CLB giữ lại → Lưu. Đơn đăng ký đi theo ca, không mất; khối của ca được giữ nguyên như trước khi chuyển. Làm ở <b>mọi đợt</b> có ca của CLB đó (đổi đợt ở ô chọn đợt).</li>
      <li>CLB không còn ca ở đợt nào sẽ có nút <b>Sửa / ẩn</b> ở dòng "CLB chưa có lớp" cuối trang → bỏ tích "Hiển thị CLB này cho phụ huynh".</li>
    </ol>
    <p style="margin:6px 0 0"><b>Không sửa khối của CLB giữ lại.</b> Ca nào không khai khối riêng sẽ đổi khối theo CLB — một ca khối 1, 2 sẽ mở cho cả khối 3, 4, 5. Mỗi ca đã giữ khối của riêng nó, CLB không cần khai đủ.</p>
  </div>`;
}


/**
 * Nhãn "Trùng tên với ..." trên khối của một CLB. CLB đã ẩn VÀ hết ca thì không gắn:
 * gắn nhãn cho nó là bảo người ta gộp tiếp một thứ đã gộp xong.
 */
...
function renderCatalogClubBlock(club, classes) {
  ...
  const rows = classes
    ...
    .map((row) => {
      // Cùng một trạng thái phụ huynh đang thấy, để giáo vụ biết phụ huynh đang thấy gì.
      const mauTrangThai = { "con-nhan": "green", "sap-du": "gold", "da-du": "red", "dung-tuyen": "purple" };
      const badge = !row.active ? ["Đang ẩn", "red"]
        : [row.trangThaiLop?.nhan || "", mauTrangThai[row.trangThaiLop?.ma] || "blue"];
      ...
      return `<tr>
        ...
        <td>${formatMoney(row.fee + (row.hocLieu || 0))}${row.hocLieu ? `<br><span style="color:var(--muted)">gồm học liệu ${formatMoney(row.hocLieu)}</span>` : ""}${row.soBuoi ? `<br><span style="color:var(--muted)">${row.soBuoi} buổi</span>` : ""}</td>
...
function classFormMarkup(clubClass, clubId) {
  ...
  const value = {
    ...
    capacity: 20, minCapacity: 0, enrolledBase: 0, fee: 0, hocLieu: 0, soBuoi: 0, nguongSapDu: 3, dangTuyen: true,
    grades: [], waitlistEnabled: true, sortOrder: 0, active: true,
    ...(clubClass || {}),
  };
  const clubOptions = sapXepClbTheoTen(state.catalog.clubs)
    .map((item) => `<option value="${escapeHtml(item.id)}" ${item.id === (clubClass?.clubId || clubId) ? "selected" : ""}>${escapeHtml(nhanClbDayDu(item, clubClass?.periodId || state.catalogPeriodId))}${item.active === false ? " · đang ẩn" : ""}</option>`).join("");
  const periodOptions = state.catalog.periods
  ...
  return `<div class="modal-head"><div><span class="eyebrow">${isNew ? "Thêm ca học" : "Chỉnh sửa ca học"}</span><h2>${escapeHtml(club?.name || "Lớp CLB")}</h2></div>
    ...
    <div class="modal-body"><form id="class-form" class="form-grid">
      ...
      <label class="form-field"><span>Học liệu (đồng)</span><input name="hocLieu" value="${value.hocLieu}" /><small class="field-hint">Phụ huynh thấy học phí tổng = học phí + học liệu.</small></label>
      <label class="form-field"><span>Số buổi học</span><input type="number" name="soBuoi" min="0" max="500" value="${value.soBuoi}" /></label>
      <label class="form-field"><span>Báo "Sắp đủ" khi còn</span><input type="number" name="nguongSapDu" min="0" max="500" value="${value.nguongSapDu}" /><small class="field-hint">chỗ trở xuống. Phụ huynh chỉ thấy trạng thái, không thấy số chỗ.</small></label>
      ...
      <label class="confirm-row form-span-2"><input type="checkbox" name="dangTuyen" ${value.dangTuyen !== false ? "checked" : ""} /><span>Đang tuyển sinh — bỏ tích thì phụ huynh vẫn thấy lớp với nhãn "Dừng tuyển" nhưng không đăng ký được</span></label>
    ...
    withBusyButton(button, "Đang lưu…", async () => {
      await api(targetId ? `/admin/classes/${encodeURIComponent(targetId)}` : "/admin/classes", {
        ...
        body: JSON.stringify({
          ...
          soBuoi: form.soBuoi,
          nguongSapDu: form.nguongSapDu,
          dangTuyen: Boolean(form.dangTuyen),
        }),
      });
      await refreshCatalog();
      closeModal();
...
async function commitCatalogImport(button) {
  ...
  try {
    ...
    toast(`Đã ghi: ${counters.clubsCreated} CLB mới, ${counters.clubsUpdated} CLB cập nhật, ${counters.classesCreated} ca mới, ${counters.classesUpdated} ca cập nhật.${counters.clubsGopVao ? ` ${counters.clubsGopVao} CLB trùng tên đã ẩn được ghi vào CLB đang mở cùng tên.` : ""}`, "success");
  } catch (error) {
    button.disabled = false;
...
const AUDIT_ACTION_LABELS = {
  ...
  PARENT_SWITCH_CLASS: "Phụ huynh đổi lớp",
...
function renderDetailStudentTab({ registration, student }) {
  ...
  return `<div class="detail-grid">
    <label class="form-field detail-status"><span>Trạng thái đăng ký</span>
      <select id="detail-status" class="select-field" ${registration.status === "doi_lop" ? "disabled" : ""}>
        ${[...LIFECYCLE_STATUSES, ...EXCEPTION_STATUSES].includes(registration.status) ? ""
          : `<option value="${escapeHtml(registration.status)}" selected disabled>${escapeHtml(statusBadge(registration.status)[0])}</option>`}
...
function renderRosterDetail() {
  ...
  const than = tab === "thong-tin"
    ? `<div class="detail-grid">
        ...
        ${detailField("Học phí", ca.moCoi ? "" : `${formatMoney(conSo(ca.fee) + conSo(ca.hocLieu))}${conSo(ca.hocLieu) ? ` (gồm học liệu ${formatMoney(conSo(ca.hocLieu))})` : ""}`)}
      ...
      <tbody>${preview.oChon.map((item) => `<tr>
        ...
        <td><select class="select-field" data-ghep-ca="${escapeHtml(item.khoa)}">
          ...
          ${(preview.nhomTheoKhoi || []).length ? `<optgroup label="Theo khối của từng em">${preview.nhomTheoKhoi.map((nhom) =>
            `<option value="${escapeHtml(nhom.dich)}" ${nhom.dich === item.dich ? "selected" : ""}>${escapeHtml(nhom.tenClb)} — ${nhom.cacCa.length} ca, chọn theo khối${nhom.cacCa.length === 1 ? " và thứ ghi trong ô" : ""}</option>`).join("")}</optgroup>` : ""}
...
function bindNhapDangKy() {
  ...
  $("#nhap-dot")?.addEventListener("change", (event) => {
    // Ca học thuộc về từng đợt: lựa chọn ghép của đợt cũ không có nghĩa ở đợt mới.
    state.nhapDangKy = { ...draft(), periodId: event.target.value, preview: null, mapping: {} };
    renderPage();
  });
  ...
  const xemTruoc = async () => {
    ...
    // Bảng ghép chỉ giữ những ô NGƯỜI VẬN HÀNH đã chọn tay (ô chọn ở bước 2 tự ghi vào
    // đây). Không chép đề xuất của máy vào: chép vào là biến một lần đoán thành "người
    // đã quyết", và lần xem sau máy không xét lại dù danh mục đã đổi — một CLB vừa có
    // thêm ca thứ hai vẫn bị dồn hết vào ca cũ.
    state.nhapDangKy = { ...d, preview, periodId: preview.periodId };
    renderPage();
  };
  ...
  $("#nhap-ghi")?.addEventListener("click", async (event) => {
    ...
    try {
      const payload = {
        ...
        soDongXepDuoc: soDon,
        // Em nào vào ca nào, đúng như bảng bước 3 vừa hiện. Máy chủ phân tích lại và
        // dừng nếu có dòng nào đổi ca, chứ không chỉ so số dòng.
        xepDuocDaXem: (d.preview?.rows || []).filter((row) => row.ketCuc === "xepDuoc")
          .map((row) => `${row.dong}|${row.studentId}|${row.classId}`),
      };
...
function addToCart(clubId) {
  ...
  // Đã đăng ký đúng lớp này thì báo; trùng giờ thì hỏi có đổi sang lớp này không.
  const daGui = donDaGuiCuaCon();
  if (daGui.some((don) => don.id === target.id)) {
    toast(`“${target.name}” đã có trong đăng ký hiện tại.`, "error");
    return;
  }
  const khongChon = lyDoKhongChonDuoc(target);
  if (khongChon) {
    toast(khongChon, "error");
    return;
  }
  const vuongGio = findScheduleConflict(target);
  if (vuongGio) {
    hoiDoiLop(target, vuongGio);
    return;
  }
  state.cart.push(clubId);
  toast(target.trangThaiLop?.ma === "da-du" ? "Đã thêm vào danh sách chờ." : "Đã thêm CLB vào giỏ đăng ký.", "success");
  renderApp();
  openCart();
}
...
function renderCart() {
  ...
  if (!items.length) {
    ...
    $("#cart-body").innerHTML = `<div class="inline-alert">${icon("spark")}<span>Hệ thống đã kiểm tra khối/lứa tuổi. Trùng lịch và quota sẽ được kiểm tra lại khi gửi.</span></div>${items.map(club => `<div class="cart-item"><span class="cart-emoji">${club.emoji}</span><div class="cart-copy"><h3>${escapeHtml(club.name)}${club.className ? ` · ${escapeHtml(club.className)}` : ""}</h3><p>${escapeHtml(club.schedule)}<br>${escapeHtml(club.room)}</p><strong>${club.trangThaiLop?.ma === "da-du" ? "Danh sách chờ" : formatMoney(club.fee)}</strong></div><button class="remove-item" data-remove="${club.id}" aria-label="Xóa">${icon("x")}</button></div>`).join("")}`;
    const total = items.filter(c => c.trangThaiLop?.ma !== "da-du").reduce((sum,c)=>sum+c.fee,0);
    $("#cart-footer").innerHTML = `<div class="summary-lines"><div class="summary-line"><span>Học sinh</span><strong>${student().name}</strong></div><div class="summary-line"><span>${items.length} lựa chọn</span><strong>${items.some(c=>c.trangThaiLop?.ma === "da-du")?"Có DS chờ":"Hợp lệ"}</strong></div><div class="summary-line total"><span>Phí dự kiến</span><strong>${formatMoney(total)}</strong></div></div><label class="confirm-row"><input id="terms-check" type="checkbox" /><span>Tôi đã kiểm tra lịch học, mức phí và đồng ý với quy định đổi/hủy của nhà trường.</span></label><button id="submit-cart" class="button button-primary" disabled>${icon("check")} Xác nhận và gửi đăng ký</button>`;
  }
  bindDrawerEvents();
}
...
function showDetail(clubId) {
  ...
  const trangThai = club.trangThaiLop || { ma: "con-nhan", nhan: "Còn nhận" };
  const khongChon = lyDoKhongChonDuoc(club);
  showModal(`<div class="modal-head"><div><span class="eyebrow">${escapeHtml(club.category)}</span><h2>Chi tiết câu lạc bộ</h2></div><button class="icon-button" data-close-modal>${icon("x")}</button></div><div class="modal-body"><div class="detail-hero"><span>${club.emoji}</span><div><h3>${escapeHtml(club.name)}${club.className ? ` · ${escapeHtml(club.className)}` : ""}</h3><p>${escapeHtml(club.description)}</p></div></div><div class="detail-grid"><div class="detail-cell"><span>Lịch học</span><strong>${escapeHtml(club.schedule)}</strong></div><div class="detail-cell"><span>Địa điểm</span><strong>${escapeHtml(club.room)}</strong></div><div class="detail-cell"><span>Giáo viên</span><strong>${escapeHtml(club.teacher)}</strong></div><div class="detail-cell"><span>Trạng thái lớp</span><strong>${escapeHtml(trangThai.nhan)}</strong></div><div class="detail-cell"><span>Khối áp dụng</span><strong>${club.grade.join(", ")}</strong></div><div class="detail-cell"><span>Học phí</span><strong>${formatMoney(club.fee)}${club.soBuoi ? ` · ${club.soBuoi} buổi` : ""}</strong></div></div></div><div class="modal-foot"><button class="button button-secondary" data-close-modal>Đóng</button><button class="button button-primary" data-modal-add="${club.id}" ${state.cart.includes(club.id) || khongChon ?"disabled":""}>${state.cart.includes(club.id)?"Đã chọn":trangThai.ma === "dung-tuyen"?"Dừng tuyển":trangThai.ma === "da-du"?(club.waitlistEnabled?"Vào DS chờ":"Đã đủ"):"Chọn CLB"}</button></div>`);
}
